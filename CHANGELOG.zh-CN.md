...
* **目录面板在输入正文时不再闪动**——此前每敲一个字、停顿后都会重建整个目录列表，目录项重画、列表自身的滚动位置被重置；现在标题结构没变就不再重建
* **点击目录项定位更准**——高亮稳定落在你点的那一项（此前常停在前一项，点第一项则完全没有高亮）；文档较短时同样准确
* **目录项整行都能点**——此前只有文字那一截有反应，行的空白处点了没反应
* **文档内锚点链接**——目标标题含中文等非 ASCII 字符、而链接是百分号编码写法时，点击不再没反应
