...
| 6 | 自定义编辑器在「打开方式」里的名字**必然是两段**「<编辑器名> - <扩展显示名>」：`editorTypePicker.ts` 的 labelWithSource 对第三方编辑器无条件拼接（`providerDisplayName` = 扩展 `displayName`，见 `contributedCustomEditors.ts`；取不到才退回扩展 id），只有内置提供方（`detail === "Built-in"`）才省略后缀——**扩展侧做不到只显示单段** | ✘（owner 按此定名，无需用户感知） | VS Code 工作台（1.137 逐行核） | 未提 issue（属上游设计，非 bug） — 因此 `contributes.customEditors[].displayName` 定为「Markdown Editor / Markdown 编辑器」，菜单行是「Markdown Editor - EPYTOR」。**别再按「让它只显示 EPYTOR」去改**：v1.1.x~v1.2.0 一直显示「EPYTOR - EPYTOR」，期间两次误判成 nls 占位符 / 「同名去重」（VS Code 无此去重）。契约由 `src/__tests__/manifest.test.ts` 钉住。**注**：「打开方式…」快速选择（`EditorResolution.PICK` 路径）是另一套渲染——label 与提供者说明分两行，那里提供者小字仍是 `EPYTOR`，这正是 2026-09-12 手测「看着没问题」的原因 |
